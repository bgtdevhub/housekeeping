  ...
  return next => action => {
    ...
    switch (action.type) {
      ...
      case LEGEND_ITEM_CLICK:
        const chartData = getTreemapData(
          store.getState().profileReducer.unchangedContent
        );
        let chartItems = [...chartData.children[0].children];
        if (!Ydnlu.isEmpty(chartData.children)) {
          chartItems = chartItems.filter(
            item => action.selectedType.name === item.name
          );
        }
        chartData.children[0].children = chartItems;
        store.dispatch({
          type: LEGEND_ITEM_CLICK_DONE,
          chart: chartData,
          mode: 'chart'
        });
        break;
